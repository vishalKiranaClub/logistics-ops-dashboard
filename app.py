    ...
    if os.path.exists(CSV_PATH):
        ...
        for col in COLUMNS:
            if col not in df.columns:
                df[col] = ""
        # coerce all numeric cols to int, filling missing/bad values with 0
        ...
        # always recompute not_dispatched from its source columns so it's never missing/stale
        df["orders_not_dispatched_on_time"] = (
            df["orders_ready_dispatch"] - df["orders_picked_up"]
...
def render_order_flow_fields(defaults=None, key_prefix="new"):
    ...
    with c1:
        orders_pending_packing = st.number_input(
            "Orders Pending for Packing",
            min_value=0,
            value=int(d.get("orders_pending_packing", 0)),
            key=f"{key_prefix}_pending",
            help="Orders that have NOT been packed at all (unpacked inventory sitting in warehouse)")
        orders_ready_dispatch = st.number_input(
            "Orders Ready for Dispatch",
            min_value=0,
            value=int(d.get("orders_ready_dispatch", 0)),
            key=f"{key_prefix}_ready",
            help="Orders that are packed and waiting to be picked up by a courier")
    with c2:
        orders_not_packed_on_time = st.number_input(
            "Orders Not Packed On Time",
            min_value=0,
            value=int(d.get("orders_not_packed_on_time", 0)),
            key=f"{key_prefix}_notpacked",
            help="Orders that missed the packing SLA — includes both packed-late and still-unpacked orders")
        orders_picked_up = st.number_input(
            "Orders Picked Up",
            min_value=0,
            value=int(d.get("orders_picked_up", 0)),
            key=f"{key_prefix}_pickedup",
            help="Orders that were successfully dispatched (picked up by courier) on the previous day")

    # Not dispatched = ready but not picked up (these are today's undispatched)
    orders_not_dispatched_on_time = max(0, int(orders_ready_dispatch) - int(orders_picked_up))
    st.markdown(
        f"""<div style="background:#0d0f14;border:1px solid #252a3a;border-left:3px solid #3b82f6;
        border-radius:8px;padding:10px 14px;margin:8px 0 4px 0;font-size:13px;color:#9ca3af;">
        📌 <b style="color:#ffffff;">Orders Not Dispatched On Time</b> &nbsp;=&nbsp;
        Ready for Dispatch − Picked Up &nbsp;=&nbsp;
        <b style="color:#ef4444;font-size:16px;">{orders_not_dispatched_on_time}</b>
        <span style="font-size:11px;margin-left:8px;">(packed orders courier did not collect)</span>
        </div>""",
        unsafe_allow_html=True
    )
...
with tab3:
    ...
    # Pickup rate = orders picked up vs orders that were ready for dispatch (same entry, previous day's dispatch)
    pickup_rate = (total_pickup / total_ready * 100) if total_ready > 0 else 0

    kpi_cols = st.columns(4)
    cards = [
        ("Pending Packing",        total_pending,         "not packed at all",          "warn"),
        ("Ready for Dispatch",     total_ready,           "packed, awaiting courier",    "accent"),
        ("Picked Up",              total_pickup,          "dispatched previous day",     "success"),
        ("Not Dispatched On Time", total_not_disp,        "ready but not collected",     "danger"),
        ("Not Packed On Time",     total_not_packed,      "missed packing SLA",          "warn"),
        ("Total Orders Impacted",  total_impacted,        "across all issues",           "danger"),
        ("Pickup Success Rate",    f"{pickup_rate:.1f}%", "picked up / ready",
    ...
    st.caption("Pickup % = Orders Picked Up ÷ Orders Ready for Dispatch  (measures courier collection rate against packed inventory)")
    ...
    stbl.columns = ["Seller","Pending Packing (unpacked)","Ready for Dispatch (packed)","Picked Up (prev day dispatched)",
